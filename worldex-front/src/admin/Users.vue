...
onMounted(fetch)
function openForm(item) { editing.value = item || null; if (item) { form.username = item.username; form.password = ''; form.role = item.role } else { form.username = ''; form.password = ''; form.role = 'editor' }; showForm.value = true }
async function handleSave() { try { const data = { role: form.role, username: form.username }; if (form.password) data.password = form.password; if (editing.value?.id) await updateUser(editing.value.id, data); else await createUser({ ...data, password: form.password || 'default' }); showForm.value = false; fetch() } catch {} }
